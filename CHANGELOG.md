...
- Updated the shipped 35B coding preset to the validated 256K local max-context configuration.
- Consolidated the March 7-8 benchmark study into the main docs so the repo now reflects the verified 35B, 27B, and 9B winners.
- Added dedicated benchmark, API, and git workflow guides for the maintained `D:` worktree layout.

### Fixed

- Stabilized the Windows 35B launcher by preferring `python` over the `py` launcher and removing the noisy redirected-input delay path.
- Hardened YAML config loading against UTF-8, UTF-8 BOM, and legacy Windows encodings.
- Switched background `llama-server` startup to a detached process launch so local server startup remains reliable under scripted use.

## [1.6.2] - 2026-03-07

### Changed

- Replaced the remaining Dependabot backlog with a consolidated dependency update for the dashboard JSON Render packages and GitHub Actions versions.
- Removed absolute local workspace paths from the public workflow docs so the maintainer workflow stays reusable and does not leak machine-specific details.

### Fixed

- Aligned package metadata and release metadata with the current published release line.
- Kept the release worktree checks self-contained by installing dashboard dependencies before audit and build, and by running Semgrep in a UTF-8-safe way on Windows.
